...
module.exports = {
     create: async (payload) => {
        try {
            const queryObject = await queryBuilder.insert(payload, 'divisions');
    ...
    fetch: async () => {
        try {
            const { rows } = await db.buildQuery(`select * from divisions`)
            return rows;
        } catch (err) {
            return new Error(ERROR_CODE.INTERNAL_SERVER_ERROR)
        }
    ...
    update: async (payload, id) => {
        try {
            ...
            const queryObject = await queryBuilder.update(payload, "divisions", whereClause)
   ...
   fetchById: async (id) => {
        try {
            const { rows: [tenant] } = await db.buildQuery(`select * from divisions where id = $1`, {id});
            return tenant;
        } catch (err) {
            return new Error(ERROR_CODE.INTERNAL_SERVER_ERROR);
        }
    ...
    delete: async (id) => {
        try {
            const { rowCount } = await db.buildQuery(`delete from divisions where id = $1`, {id});
            return { rowCount };
        } catch (err) {
            return new Error(ERROR_CODE.INTERNAL_SERVER_ERROR);
        }
