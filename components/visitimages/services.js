...
module.exports = {
     create: async (payload) => {
        try {
            const queryObject = await queryBuilder.insert(payload, 'visitimages');
    ...
    fetch: async () => {
        try {
            const { rows } = await db.buildQuery(`select * from visitimages`)
            return rows;
        } catch (err) {
            return new Error(ERROR_CODE.INTERNAL_SERVER_ERROR)
        }
    ...
    update: async (payload, id) => {
        try {
            ...
            const queryObject = await queryBuilder.update(payload, "visitimages", whereClause)
   ...
   fetchById: async (id) => {
        try {
            const { rows: [tenant] } = await db.buildQuery(`select * from visitimages where id = $1`, {id});
            return tenant;
        } catch (err) {
            return new Error(ERROR_CODE.INTERNAL_SERVER_ERROR);
        }
    ...
    delete: async (id) => {
        try {
            const { rowCount } = await db.buildQuery(`delete from visitimages where id = $1`, {id});
            return { rowCount };
        } catch (err) {
            return new Error(ERROR_CODE.INTERNAL_SERVER_ERROR);
        }
